package nl.tudelft.trustchain.idelft2.ui.contacts
...
import nl.tudelft.trustchain.idelft2.databinding.FragmentContactsChatBinding
import nl.tudelft.trustchain.idelft2.dialogs.*
...
import nl.tudelft.trustchain.idelft2.util.*
...
class ContactChatFragment : VTFragment(R.layout.fragment_contacts_chat) {
    ...
    private val publicKeyBin by lazy {
        requireArguments().getString(idelft2MainActivity.ARG_PUBLIC_KEY)!!
    }

    private val name by lazy {
        requireArguments().getString(idelft2MainActivity.ARG_NAME)!!
    }

    private val parentTag by lazy {
        requireArguments().getString(idelft2MainActivity.ARG_PARENT)!!
    }
    ...
    private fun goToContactFragment(contact: Contact) {
        ...
        contactChatFragment.arguments = Bundle().apply {
            putString(idelft2MainActivity.ARG_PUBLIC_KEY, contact.publicKey.keyToBin().toHex())
            putString(idelft2MainActivity.ARG_NAME, contact.name)
            putString(idelft2MainActivity.ARG_PARENT, parentTag)
        }
